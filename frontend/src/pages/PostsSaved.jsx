...
import { getSavePostsdAPI, searchPostSavedAPI } from "../services/postService";
...
const PostsSaved = () => {
  ...
  const getData = async () => {
    ...
    try {
      let data;
      if (keywordSearch) {
        data = await searchPostSavedAPI(1, keywordSearch);
      } else {
        data = await getSavePostsdAPI(1);
      }
  ...
  const handlePageClick = async (selectedPage) => {
    ...
    try {
      let data;
      if (keywordSearch) {
        data = await searchPostSavedAPI(
          selectedPage.selected + 1,
          keywordSearch
        );
      } else {
        data = await getSavePostsdAPI(selectedPage.selected + 1);
      }
